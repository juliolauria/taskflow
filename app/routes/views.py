...
def build_tasks_context(
    ...
    # The sidebar highlights a specific project only when the page is
    # scoped to exactly that project and nothing else — otherwise it
    # falls back to the generic "All Tasks" entry.
    if len(projects) == 1 and not priorities and not statuses and not responsibles and not group_by:
        sidebar_filter = projects[0]
    else:
        sidebar_filter = "all"
...
def active_view(
    ...
    with SessionLocal() as db:
        context = build_tasks_context(
            db, request, group_by, projects, priorities, statuses, responsibles
        )
